...
async function main() {
  ...
  const output = path.join(process.cwd(), 'public', 'hero-sequence-v6');
  ...
  for (const [index, file] of files.entries()) {
    ...
    const desktop = await sharp(input).resize(3840,2160).webp({ quality: 96, effort: 6, smartSubsample: true }).toFile(path.join(output, `${name}.webp`));
    const mobile = await sharp(input).resize(1280,720).webp({ quality: 90, effort: 6, smartSubsample: true }).toFile(path.join(output, `${name}-mobile.webp`));
    desktopBytes += desktop.size;
    mobileBytes += mobile.size;
  }
  const report = { frames: files.length, sourceBytes, desktopBytes, mobileBytes, desktopDimensions: [3840, 2160], mobileDimensions: [1280, 720], desktopQuality: 96, mobileQuality: 90, assetVersion: 6 };
